...
news_categories = ['MLB', '中職', '日職', '三級棒球', '全部']
...
def get_keyword_time_based_freq(df_query):
    ...
    # 創建包含日期和頻率的 DataFrame
    query_freq = pd.DataFrame({'date_index': date_samples, 'freq': 1})
    ...
    data = data.reindex(date_range, fill_value=0).reset_index()
    
    time_data = []
    for i in range(len(data)):
        row = {
            'x': data['index'].iloc[i].strftime('%Y/%m/%d'), 
            'y': int(data['freq'].iloc[i])  # 直接從 DataFrame 取得頻率值
        }
        time_data.append(row)
    return time_data
